...
| creator | text | - | 제작자 |
| creator_icon | text | - | 제작자 이미지 URL (500) |
| product_code | text | UNIQUE (nullable) | 주소/연결용 상품코드 |
| content | text | - | 내용 |
| price | integer | - | 가격 (원) |
...
create unique index colors_product_code_key on colors (product_code) where product_code is not null;
```

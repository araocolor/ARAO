...
## 세션 작업 원칙 메모 (2026-04-15)

- 질문만 받은 경우: 실행하지 말고 답변만 한다.
- 사용자 승인 없이 코드 수정/서버 실행/캐시 삭제를 하지 않는다.
- 개발 중에는 `npm run build`를 기본적으로 생략한다.
- 빌드는 커밋/배포 직전에만 1회 실행한다.
- 내가 할 일은 `하겠습니다`로 말하고, 사용자에게 지시형(`하세요`) 표현을 쓰지 않는다.
